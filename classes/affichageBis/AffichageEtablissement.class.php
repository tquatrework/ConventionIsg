...
namespace Affichage;
use \PDO;
class AffichageEtablissement extends Affichage{
    ...
    public function __construct($dbh){
        ...
        $tabResult = $this->select($dbh,"Liste des établissements d'enseignement","Aucun établissement");
        ...
        if(is_array($tabResult) && !empty($tabResult)){
            foreach($tabResult as $tab){
                ...
                $this->lien_suppression = "index.php?id_etablissement=".$tab["id_etablissement"]."&controller=etablissement&task=supprimer";
                $this->card();
                $this->reset();
                echo "<br/>";
            }
